...
let val_400 = value_int 400
let val_500 = value_int 500

(* Test case 4: Delete after two instances of the same field in different records *)
let () =
  ...
  assert (Obj.get obj bar pc = [ (ite (eq bar x) val_300 val_200, pc) ]);
  assert (Obj.get obj age pc = [ (ite (eq age x) val_300 val_10, pc) ]);
  assert (Obj.get obj x pc = [ (val_300, pc) ]);
  assert (Obj.get obj banana pc = [ (ite (eq banana x) val_300 undef, pc) ]);
  assert (
    Obj.get obj z pc
    = [ ( ite (eq z x) val_300
            (ite (eq z bar) val_200
               (ite (eq z foo) val_100 (ite (eq z age) val_10 undef)) )
        , pc )
      ] );
  ...
  assert (
    ...
    = [ (ite (eq banana y) val_500 (ite (eq banana x) val_300 undef), pc) ] );
  assert (
    Obj.get obj z pc
    = [ ( ite (eq z y) val_500
            (ite (eq z foo) val_400
  ...
  assert (
    Obj.has_field obj bar pc
    = ite (eq bar x) (value_bool true)
        (ite (eq bar y) (value_bool true) (value_bool true)) );
  assert (
    Obj.has_field obj age pc
    = ite (eq age x) (value_bool true)
  ...
  assert (
    Obj.get obj bar pc
    = [ (ite (eq bar x) val_300 (ite (eq bar y) val_500 val_200), pc) ] );
  assert (
    Obj.get obj age pc
...
(* Test case 4.1: Delete after two instances of the same field in different records with pc = (foo != x)*)
let () =
  let pc = ne foo x in
