...
let () =
  ...
  *)
  ...
  (* test get *)
  assert (
    Obj.get merged_obj a pc = [ (ite (and_ (eq a x) cond) val_1 val_3, pc) ] );
  assert (
    Obj.get merged_obj b pc = [ (ite (and_ (eq b x) cond) val_1 undef, pc) ] );
  assert (
    Obj.get merged_obj c pc
    = [ (ite (and_ (eq c x) cond) val_1 (ite (not_ cond) val_2 undef), pc) ] );
  assert (
  ...
  assert (
    ...
    = [ ( ite
            (and_ (eq z x) cond)
            val_1
            (ite (and_ (eq z c) (not_ cond)) val_2 (ite (eq z a) val_3 undef))
        , pc )
      ] )
