...
let () =
  ...
  o2 = {}         -> Heap.alloc  
   ...
   -----> o  = {{d:6};_;0}; {If #y > 3 then {{b:7};_;1} else Empty;{{};#z:5;2}; {Empty}{{a:3}; #x:4 ; 0}}
  ...
  *)
  ...
  let _ = assert (Mem.get then_mem loc2 b pc = [ (undef, pc) ]) in
  ...
  assert (
    ...
    = [ (ite (and_ (eq w z) (not_ cond)) val_7 undef, pc) ] )
